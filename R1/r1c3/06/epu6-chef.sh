...
$CMDPREFIX mv bootconf.json /opt/dt-data/run/epu6chefroles.json
if [ $? -ne 0 ]; then
  exit 1
fi
...
$CMDPREFIX mv chefconf.rb /opt/dt-data/run/epu6chefconf.rb
if [ $? -ne 0 ]; then
  exit 1
fi

cat >> rerun-epu6chef.sh << "EOF"
#!/bin/bash
CHEFLEVEL="info"
if [ "X" != "X$1" ]; then
  CHEFLEVEL=$1
...
rm -rf /home/epu6/app
rm -rf /home/epu6/app-venv
chef-solo -l $CHEFLEVEL -c /opt/dt-data/run/epu6chefconf.rb -j /opt/dt-data/run/epu6chefroles.json
exit $?
EOF

chmod +x rerun-epu6chef.sh
if [ $? -ne 0 ]; then
  exit 1
fi

$CMDPREFIX mv rerun-epu6chef.sh /opt/rerun-epu6chef.sh
if [ $? -ne 0 ]; then
  exit 1
fi

echo "Running chef-solo"
$CMDPREFIX /opt/rerun-epu6chef.sh  #debug
if [ $? -ne 0 ]; then
  exit 1
fi
