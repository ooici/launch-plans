...
$CMDPREFIX mv bootconf.json /opt/dt-data/run/epu10chefroles.json
if [ $? -ne 0 ]; then
  exit 1
fi
...
$CMDPREFIX mv chefconf.rb /opt/dt-data/run/epu10chefconf.rb
if [ $? -ne 0 ]; then
  exit 1
fi

cat >> rerun-epu10chef.sh << "EOF"
#!/bin/bash
CHEFLEVEL="info"
if [ "X" != "X$1" ]; then
  CHEFLEVEL=$1
fi
rm -rf /home/epu10/app
rm -rf /home/epu10/app-venv
chef-solo -l $CHEFLEVEL -c /opt/dt-data/run/epu10chefconf.rb -j /opt/dt-data/run/epu10chefroles.json
exit $?
EOF

chmod +x rerun-epu10chef.sh
if [ $? -ne 0 ]; then
  exit 1
fi

$CMDPREFIX mv rerun-epu10chef.sh /opt/rerun-epu10chef.sh
if [ $? -ne 0 ]; then
  exit 1
fi

echo "Running chef-solo"
$CMDPREFIX /opt/rerun-epu10chef.sh  #debug
if [ $? -ne 0 ]; then
  exit 1
fi
