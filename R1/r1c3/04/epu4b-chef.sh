...
$CMDPREFIX mv bootconf.json /opt/dt-data/run/epu4bchefroles.json
if [ $? -ne 0 ]; then
  exit 1
fi
...
$CMDPREFIX mv chefconf.rb /opt/dt-data/run/epu4bchefconf.rb
if [ $? -ne 0 ]; then
  exit 1
fi

cat >> rerun-epu4bchef.sh << "EOF"
#!/bin/bash
CHEFLEVEL="info"
if [ "X" != "X$1" ]; then
  CHEFLEVEL=$1
fi
rm -rf /home/epu4b/app
rm -rf /home/epu4b/app-venv
chef-solo -l $CHEFLEVEL -c /opt/dt-data/run/epu4bchefconf.rb -j /opt/dt-data/run/epu4bchefroles.json
exit $?
EOF

chmod +x rerun-epu4bchef.sh
if [ $? -ne 0 ]; then
  exit 1
fi

$CMDPREFIX mv rerun-epu4bchef.sh /opt/rerun-epu4bchef.sh
if [ $? -ne 0 ]; then
  exit 1
fi

echo "Running chef-solo"
$CMDPREFIX /opt/rerun-epu4bchef.sh  #debug
if [ $? -ne 0 ]; then
  exit 1
fi
